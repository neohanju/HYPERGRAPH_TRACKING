...
void CEvaluator::Initialize(std::string strFilepath)
{
	...
	this->m_rectCropZone.width = CROP_ZONE_X_MAX - CROP_ZONE_X_MIN;
	this->m_rectCropZone.height = CROP_ZONE_Y_MAX - CROP_ZONE_Y_MIN;

	this->m_rectCropZoneMargin = this->m_rectCropZone;
	this->m_rectCropZoneMargin.x -= CROP_ZONE_MARGIN;
	this->m_rectCropZoneMargin.x -= CROP_ZONE_MARGIN;
	this->m_rectCropZoneMargin.width += 2 * CROP_ZONE_MARGIN;
	this->m_rectCropZoneMargin.height += 2 * CROP_ZONE_MARGIN;
...
void CEvaluator::SetResult(std::vector<pointInfo> &points, int timeIdx)
{
	if (timeIdx > this->m_queueSavedResult.size()) { return; }

	this->m_queueSavedResult[timeIdx].clear();
	cv::Rect2d containingZone = BOUNDARY_PROCESSING_? m_rectCropZoneMargin : m_rectCropZone;
	for (int pIdx = 0; pIdx < points.size(); pIdx++)
	{		
		int curId = points[pIdx].first;
		if (!containingZone.contains(points[pIdx].second))
		{ continue; }

		// index management
		int indexPos = 0;
		std::deque<unsigned int>::iterator findIter = std::find(this->m_queueID.begin(), this->m_queueID.end(), curId);
		if (this->m_queueID.end() == findIter)
		{
			this->m_queueID.push_back(curId);
		...
		this->m_queueSavedResult[timeIdx].push_back(points[pIdx]);
	}
	this->m_nSavedResult++;
}
	...
	try
	{
		...
		for (int timeIdx = 0; timeIdx < this->m_nNumTime; timeIdx++)
		{
			for (int objIdx = 0; objIdx < numObject; objIdx++)
			{
				...
				if (!bFound)
				{
					this->m_queueSavedResult[timeIdx].push_back(std::make_pair(objIdx, cv::Point2d(0.0, (double)readingFloat)));
				}
			}
			fscanf_s(fp, "\n");
		}
		...
		for (int timeIdx = 0; timeIdx < this->m_nNumTime; timeIdx++)
		{
			...
			pointInfoIter++)
			{
				cv::Point2d curPoint((*pointInfoIter).second.x, (*pointInfoIter).second.y);			
				if ((BOUNDARY_PROCESSING_ && !this->m_rectCropZoneMargin.contains(curPoint))
					|| (!BOUNDARY_PROCESSING_ && !this->m_rectCropZone.contains(curPoint))) 
				{ continue; }
...
void CEvaluator::Evaluate(void)
{
	...
	//---------------------------------------------------------	
	// zone setting
	cv::Rect2d rectInnerCropZone = this->m_rectCropZone;
	rectInnerCropZone.x += CROP_ZONE_MARGIN;
	rectInnerCropZone.y += CROP_ZONE_MARGIN;
	rectInnerCropZone.width -= 2.0 * CROP_ZONE_MARGIN;
	rectInnerCropZone.height -= 2.0 * CROP_ZONE_MARGIN;
	...
	for (int timeIdx = 0; timeIdx < this->m_nNumTime; timeIdx++)
	{
			...
			pointInfoIter++)
		{
			...
			if (!this->m_rectCropZone.contains(curPoint)) { continue; }
			queueCroppedResult[timeIdx].push_back(*pointInfoIter);

			if (!rectInnerCropZone.contains(curPoint)) { continue; }
	...
	//{		
	//	cv::Point2d curPoint;
	//	cv::Point2d prevPoint, nextPoint;
	...
	//				if(this->m_rectCropZone.contains(prevPoint))
	//				{
	//					bPrevIn = true;
	//				}
	//			}
	...
	//				if (this->m_rectCropZone.contains(nextPoint))
	//				{
	//					bNextIn = true;
	//				}
	//			}
	...
	cv::Mat newXgtCandidate = this->matXgt(cv::Rect2d(0.0, 0.0, this->matXgt.cols, newNumTime)).clone().t();
	cv::Mat newYgtCandidate = this->matYgt(cv::Rect2d(0.0, 0.0, this->matYgt.cols, newNumTime)).clone().t();
	...
	cv::Point2d curGtPoint;
	cv::Point2d curResPoint;
	for (int t = 0; t < F; t++)
	{
		...
		if (t > 0)
		{
			for(int mapIdx = 0; mapIdx < (int)M.cols; mapIdx++)
			{
				...
				if (std::sqrt((curGtPoint - curResPoint).dot(curGtPoint - curResPoint)) > MAXIMUM_POINT_ERROR) { continue; }
		...
		do
		{
			...
			for (int o = 0; o < (int)GTsNotMapped.size(); o++)
			{
				...
				for (int e = 0; e < (int)EsNotMapped.size(); e++)
				{
					...
					curDist = std::sqrt((GT - E).dot(GT - E));
					if (curDist < minDist)
					{
						minDist = curDist;
		...
		for (int ct = 0; ct < Ngt; ct++)
		{
			...
			d.at<double>(t, ct) = std::sqrt((curGtPoint - curResPoint).dot(curGtPoint - curResPoint));
		}
